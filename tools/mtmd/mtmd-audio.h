...
struct mtmd_audio_preprocessor_dasheng : mtmd_audio_preprocessor {
    mtmd_audio_preprocessor_dasheng(const clip_ctx * ctx) : mtmd_audio_preprocessor(ctx) {}
    void initialize() override;
    bool preprocess(const float * samples, size_t n_samples, std::vector<mtmd_audio_mel> & output) override;

  private:
    mtmd_audio_cache cache;     // semantic branch
    mtmd_audio_cache cache_ac;  // acoustic branch, DashengTokenizer only
};
